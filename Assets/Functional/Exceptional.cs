...
public struct Exceptional<T>
{
    ...
    public static implicit operator Either<Exception, T>(Exceptional<T> original)
        => original.Exception ? (Either<Exception, T>)Left<Exception>(original.Ex) : Right<T>(original.Value);
    ...
    public Either<Exception, T> AsEither()
        => Exception ? (Either<Exception, T>)Left<Exception>(Ex) : Right<T>(Value);
